...
type HeadSupported interface {
	Head(*http.Request, http.Header, []byte) (int, interface{}, http.Header)
}

// PatchSupported is the interface that provides the Patch
// method a resource must support to receive HTTP PATCHs.
type PatchSupported interface {
	Patch(*http.Request, http.Header, []byte) (int, interface{}, http.Header)
}
...
func (api *API) requestHandler(resource interface{}, path string) http.HandlerFunc {
	return func(rw http.ResponseWriter, request *http.Request) {
		...
		var handler func(*http.Request, http.Header, []byte) (int, interface{}, http.Header)
		...
		paramsBytes, err := ioutil.ReadAll(request.Body)
		// var controllerName string
		// if t := reflect.TypeOf(resource); t.Kind() == reflect.Ptr {
		// 	controllerName = "*" + t.Elem().Name()
		// } else {
		...
		c, err := ioutil.ReadAll(request.Body)
		fmt.Println(string(c))
		code, data, header := handler(request, request.Header, paramsBytes)
...
func (api *API) AddResource(resource interface{}, paths ...string) {
	for _, path := range paths {
		api.Mux().HandleFunc(path, api.requestHandler(resource, path))
	}
}
...
func (api *API) AddResourceWithWrapper(resource interface{}, wrapper func(handler http.HandlerFunc) http.HandlerFunc, paths ...string) {
	for _, path := range paths {
		api.Mux().HandleFunc(path, wrapper(api.requestHandler(resource, path)))
	}
}
